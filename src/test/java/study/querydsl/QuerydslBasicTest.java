...
*/
    ...
    public void concat() {
        ...
        // {username}_{age}
        List<String> result = queryFactory
                .select(member.username.concat("_").concat(member.age.stringValue())) // stringValue() 쓸 일 많음.
                .from(member)
                .where(member.username.eq("member1"))
                .fetch();
    ...
    public void findDtoByJPQL() {
        // 타입을 맞춰주기 위해 new 오퍼레이터를 사용해서 패키지 명을 적어준다.
    ...
    @Test
    public void dynamicQuery_BoolenaBuilder() {
    ...
    // 이 방법이 위 searchMember1 보다 가독성 좋은 더 깔끔한 코드
    private List<Member> searchMember2(String usernameCond, Integer ageCond) {
        return queryFactory
                .selectFrom(member)
                .where(usernameEq(usernameCond), ageEq(ageCond)) // where param 에 null 이 들어가면 조건이 무시됨(뺌)
